...
pub async fn handle(
    ...
    // Copy headers (skip host — hyper sets it from URI)
    ...
    *state.last_latency_ms.lock().unwrap() = latency_ms;
    ...
    // Build response — copy status and headers (skip hop-by-hop)
    ...
    // Collect upstream body and send it back
    // NOTE: We collect the full body here. For very large streaming responses,
    // a frame-by-frame approach would be better, but Claude Code API responses
    // are typically small enough that this works fine.
...
mod tests {
    ...
    #[test]
    fn test_capture_partial_headers() {
        let mut headers = HeaderMap::new();
        headers.insert("anthropic-ratelimit-unified-5h-utilization", "0.5".parse().unwrap());

        let entry = capture_headers(&headers).unwrap();
        assert_eq!(entry.five_h_utilization, Some(0.5));
        assert!(entry.seven_d_utilization.is_none());
        assert!(entry.five_h_reset.is_none());
    }
    ...
    #[test]
    fn test_capture_malformed_utilization() {
        let mut headers = HeaderMap::new();
        headers.insert("anthropic-ratelimit-unified-5h-utilization", "not-a-number".parse().unwrap());
        headers.insert("anthropic-ratelimit-unified-status", "allowed".parse().unwrap());

        let entry = capture_headers(&headers).unwrap();
        assert!(entry.five_h_utilization.is_none()); // parse failed
        assert_eq!(entry.status.as_deref(), Some("allowed"));
    }
